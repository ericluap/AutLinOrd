...
/-!
  This file defines `ExtendsLeft`, `ExtendsRight`, and `ExtendsLeftRight` which
  represent different ways that two linear orders can intersect. It then proves
  different facts about their intersection in each case.
-/
...
def intersect_extendsLeftRight_initial
    ...
    (extendsLeftRight : ExtendsLeftRight A B) : (A ∩ B : Set α) ≤i B :=
  extendsLeft_intersect_initial A B A_interval extendsLeftRight.1
...
def intersect_extendsLeftRight_final
    ...
    (extendsLeftRight : ExtendsLeftRight A B) : (A ∩ B : Set α)ᵒᵈ ≤i Aᵒᵈ :=
  extendsRight_intersect_final A B B_interval extendsLeftRight.2
